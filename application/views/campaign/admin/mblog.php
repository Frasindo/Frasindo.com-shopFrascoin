        ...
        <div class="col-md-12">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Blog Campaign</h3>
            </div>
            <div class="box-body">
                  ...
                  <tbody>
                    <?php foreach ($list->result() as $key => $value): ?>
                      <tr>
                        ...
                        <td style="width:15%;"><?= ($value->status == 1)?"<button class='btn btn-warning reject' data-id='".$value->id_bc."' data-type='0'>Reject</button>":"<button class='btn btn-success accept' data-id='".$value->id_bc."' data-type='1'>Accept</button>" ?> <button class='btn btn-danger delete' data-id='<?= $value->id_bc ?>'>Delete</button</td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                </table>
    ...
    bootbox.confirm('<div class="row"><div class="col-md-12"><center><h4>Are You Sure ? </h4></center></div></div>',function(d){
      if(d)
      {
        ...
        dialog.init(function(){
            setTimeout(function(){
              $.post(base_url+"rest/deleteblog/",datapost,function(a){
                if(a.status == 1)
                {
                  ...
                  setTimeout(function(){window.location = "<?= base_url("campaign/admin/blogger") ?>";},2000);
                }else{
  ...
  $(".table").on("click",".reject",function(){
    ...
    bootbox.confirm('<div class="row"><div class="col-md-12"><center><h4>Are You Sure ? </h4></center></div></div>',function(d){
      if(d)
      {
        ...
        dialog.init(function(){
            setTimeout(function(){
              $.post(base_url+"rest/updateblog/",datapost,function(a){
                if(a.status == 1)
                {
                  dialog.find(".modal-title").html("Request Successfully");
                  dialog.find('.bootbox-body').html('<div class="alert alert-success"><center>'+a.msg+'</center></div>');
